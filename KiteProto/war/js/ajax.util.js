...
var ENTITY_CONTENT='content';
var ENTITY_CIRCLE='circle';
var ENTITY_LOGIN='login';
var ENTITY_AUTOSARTIFACT='autosartifact';
...
//function to add an entity when customer clicks on the add button in UI
...
//function to search an entity when customer inputs the value in the search box
...
//added KT - function to display circles for the selected customer
var showCircles = function(entity) {
	$('.message').hide();
	 //open the circles display page for this user
	window.location.href = 'circle.html#circle';
}
...
//function called when customer clicks on the cancel button
...
var getData=function(url,filterData,successFn,errorFn){
	...
	$.ajax({
		...
		data:filterData,
		success : function(resp) {
			//calling the customer defined success function
			if(successFn)
			successFn(resp);	
		},
	error:function(e){
		//calling the customer defined error function
		if(errorFn)
		 errorFn(e);
	}
	});
		...
		if(data.length > 0){
			for (var i=0;i<data.length;i++){
				...
				case ENTITY_ORDER:
					...
					break;
				case ENTITY_CUSTOMER:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].firstName+'</td><td>'+data[i].lastName+'</td><td>'+data[i].intrCategories+'</td><td>'+data[i].eMail+'</td>';
					break;
				case ENTITY_CONTENT:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].contentid+'</td><td>'+data[i].categoryid+'</td>';
					break;			
				case ENTITY_CIRCLE:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].circleid+'</td><td>'+data[i].categoryid+'</td>';
					break;		
				case ENTITY_AUTOSARTIFACT:
					htm+='<td>'+data[i].name+'</td><td>'+data[i].autosartifactid+'</td><td>'+data[i].categoryid+'</td>';
					break;						
				default:
					htm+=""; 
				}
